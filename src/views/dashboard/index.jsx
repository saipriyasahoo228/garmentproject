...
// const apiEndpoints = [
//   { key: 'daily', title: 'Daily Sales', url: '/api/retailsale/dailysales/' },
//   { key: 'monthly', title: 'Monthly Sales', url: '/api/retailsale/monthlysales/' },
//   { key: 'yearly', title: 'Yearly Sales', url: '/api/retailsale/yearlysales/' },
// ];
...
//               const { title, current_date, current_day_sales } = res.data;
...
//               const { title, current_month, current_month_sales } = res.data;
...
//               const { title, current_year, current_year_sales } = res.data;
...
//               <Card.Body>
//                 <h5>{data.title}</h5>
//                 {data.date && <p>Date: <strong>{data.date}</strong></p>}
//                 {data.month && <p>Month: <strong>{data.month}</strong></p>}
//                 {data.year && <p>Year: <strong>{data.year}</strong></p>}
//                 <p>Total Amount: <strong>${data.total_amount}</strong></p>
...
//   // Target values for progress bars (in INR)
//   const targets = {
//     daily: 500,
//     monthly: 4000,
//     yearly: 700,
...
//       <Row>
//         {salesData.map((data, index) => {
//           const progress = Math.min((data.total_amount / targets[data.key]) * 100, 100); // Calculate progress percentage
//           let progressBarVariant;

//           // Set progress bar color based on progress value
//           if (progress < 50) {
//             progressBarVariant = 'danger'; // Red
//           } else if (progress >= 50 && progress < 75) {
//             progressBarVariant = 'warning'; // Orange
//           } else if (progress >= 75 && progress < 100) {
//             progressBarVariant = 'primary'; // Blue
//           } else {
//             progressBarVariant = 'success'; // Green
//           }

//           // Subtle color schemes for card background and border
//           const cardBodyStyle = {
//             backgroundColor: '#f0f8ff', // Light blue background for the body
//             color: '#333', // Dark text color
//             border: '1px solid #dcdcdc', // Light gray border for a subtle effect
//           };

//           const cardHoverStyle = {
//             backgroundColor: '#e6f7ff', // Slightly darker blue on hover
//           };
...
import React, { useEffect, useState } from 'react';
import { Row, Col, Card, ProgressBar, Modal, Button, Table } from 'react-bootstrap';
import api from "../../api";
...
const DashDefault = () => {
  ...
  // Handle progress bar click to show modal
  ...
  return (
    <React.Fragment>
      <Row>
        {salesData.map((data, index) => {
          ...
          if (progress < 50) {
            ...
            // Monthly sales will turn yellow here
            progressBarVariant = data.key === 'monthly' ? 'warning' : 'primary'; // Orange/Yellow for monthly
          } else if (progress >= 75 && progress < 100) {
            progressBarVariant = 'primary'; // Blue
          } else {
            progressBarVariant = 'success'; // Green
              ...
              <Card>
                <Card.Body>
                  ...
                  <ProgressBar
                    ...
                    title={`Progress: ${progress.toFixed(1)}%`}
                    variant={progressBarVariant}
                    onClick={() => handleProgressClick(data)} // Show modal on click
                  />
                </Card.Body>
              </Card>
            </Col>
      ...
      {/* Modal for displaying sales details */}
      {modalData && (
        <Modal show={showModal} onHide={handleCloseModal} size="lg">
          <Modal.Header closeButton>
            <Modal.Title>{modalData.title} Sales Details</Modal.Title>
          </Modal.Header>
          <Modal.Body>
