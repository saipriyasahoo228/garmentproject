...
//     setCategoryDetails(categoryList[index]);
//     setEditIndex(index);
//     setOpen(true);
//   };
...
//         const response = await api.put(`/api/user/categories/${categoryDetails.category_code}/`, categoryDetails);
//         console.log('Updated Category Data:', response.data.data); // Log the response for debugging
  
//         const updatedList = [...categoryList];
//         updatedList[editIndex] = response.data.data; // Ensure updated data has the date field
//         setCategoryList(updatedList);
//         setEditIndex(null); // Clear editIndex
//       } else {
//         // Add new category
//         const response = await api.post('/api/user/categories/', categoryDetails);
...
//     try {
//       await api.delete(`/api/user/categories/${categoryDetails.category_code}/` , categoryDetails);
...
export default function SimplePaper() {
  ...
  {Object.values(categoryList).map((category) => (
    ...
    <TableRow key={category.category_code}>
      ...
      {/* <TableCell>
  {console.log("Subcategory Name Array:", category.sub_category_name)}
  {Array.isArray(category.sub_category_name) && category.sub_category_name.length > 0
    ? category.sub_category_name.map((sub) => {
        console.log("Subcategory Name:", sub.name); // Log each subcategory name
        return sub.name;
      }).join(", ")
    : "No Subcategories"}
</TableCell> */}
